...
import { degrees, PDFDocument, rgb, StandardFonts, PDFField, PDFFont, PDFImage, PDFForm, PDFPage } from 'pdf-lib';
...
export class PdfDocumentSaver {
  ...
  private async embedImages(overlays: Overlays, pdfDoc: PDFDocument): Promise<Map<string, PDFImage>> {
    ...
    for (const [pageNumber, pageOverlays] of overlays.pagesOverlays) {
        for (const imageOverlay of pageOverlays.imageOverlays) {
            ...
            const pdfImage = imageOverlay.imageType == ImageType.PNG ? await pdfDoc.embedPng(imageOverlay.base64)
                : imageOverlay.imageType == ImageType.JPEG ? await pdfDoc.embedJpg(imageOverlay.base64)
                    : null;
            if (pdfImage == null) {
                console.log(`Error embedding images in PDF - invalid image format. ${imageOverlay}`);
                continue;
            }
    ...
    private async doImageDrawing(pageOverlays: PageOverlays, pdfDoc: PDFDocument, page: PDFPage, base64ToPdfImageMap: Map<string, PDFImage>) {
        for (const imageOverlay of pageOverlays.imageOverlays) {
            const pdfImage = base64ToPdfImageMap.get(imageOverlay.base64);
            if (pdfImage == null) {
                console.log(`Error reading pdfPage - missing mapping for base64. ${imageOverlay}`);
                continue;
            }
    ...
    private doTextDrawing(pageOverlays: PageOverlays, page: PDFPage, neededFonts: Map<string, PDFFont>, helveticaFont: PDFFont) {
    ...
    private populateFormValues(formInputValues: FormInputValues, form: PDFForm) {
        for (const [key, value] of formInputValues.textNameToValue) {
            try {
                ...
                console.log(`Error thrown while updating the text field value of ${key} with ${value}.`, error)
            }
        }
        ...
        for (const [key, value] of formInputValues.checkboxNameToValue) {
            try {
                ...
                console.log(`Error thrown while updating the checkbox value of ${key} with ${value}.`, error)
            }
        }
        ...
        for (const [key, value] of formInputValues.dropdownNameToSelectedIndex) {
            try {
                ...
                console.log(`Error thrown while updating the dropdown value of ${key} with ${value}.`, error)
            }
        }
        ...
        for (const [key, value] of formInputValues.optionNameToSelectedIndex) {
            try {
                ...
                console.log(`Error thrown while updating the options list value of ${key} with ${value}.`, error)
            }
        }
        ...
        for (const [key, value] of formInputValues.radioGroupNameToSelectedIndex) {
            try {
                ...
                console.log(`Error thrown while updating the radio group value of ${key} with ${value}.`, error)
            }
        }
    }
